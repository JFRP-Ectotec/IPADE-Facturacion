...
SELECT REPLACE('Juan Fernando', 'n', '')
FROM dual
;

SELECT *
FROM tzrpofi
WHERE tzrpofi_pidm = gb_common.f_get_pidm('A00085240')
	AND tzrpofi_docnum_pos = 56
;

-- Revisar luego el calculo de impuestos.
DECLARE
	datos_banner CLOB;
	matricula VARCHAR2(20 CHAR) := 'A00085307';
	tran_number NUMBER := 98;
	vlt_respuesta TY_TRALIX_ENVIOFAC_RESPONSE;
	num_linea NUMBER := 1;
	tran_original NUMBER := 96;
	tran_impuestos NUMBER := NULL;
	desc_original VARCHAR2(100 CHAR) := 'S';
	fecha_emision DATE := TO_DATE('17-MAR-2026', 'DD-MON-YYYY');
BEGIN
...
WHERE gurdbug_value LIKE '%mat%A00085277%'
    ...
    --AND*/ gurdbug_activity_date > TO_DATE('17-MAR-2026 15:15:00', 'DD-MON-YYYY HH24:MI:SS')
    -- AND gurdbug_activity_date < TO_DATE('17-MAR-2026 15:17:00', 'DD-MON-YYYY HH24:MI:SS') 
ORDER BY gurdbug_activity_date DESC
;

SELECT *
FROM gurdbug
WHERE /*gurdbug_value LIKE '%mat%A00085307%'
    -- AND gurdbug_parm LIKE '%fn_cancel%'
    -- AND*/ gurdbug_activity_date > TO_DATE('18-MAR-2026 17:44:00', 'DD-MON-YYYY HH24:MI:SS')
    AND gurdbug_activity_date < TO_DATE('18-MAR-2026 17:46:00', 'DD-MON-YYYY HH24:MI:SS') 
ORDER BY gurdbug_activity_date DESC
;
...
SELECT tbraccd_pidm, tbraccd_tran_number, tbraccd_detail_code, 
	tbraccd_amount, tbraccd_effective_date, tbraccd_receipt_number,
	tbraccd_tran_number_paid
FROM tbraccd
WHERE tbraccd_pidm = gb_common.f_get_pidm('A00085277')
	AND tbraccd_tran_number = 171
;
